...
object App extends App {
  ...
  def postTransactions(a: Account,
                       creditAmount: Amount,
                       debitAmount: Amount): Kleisli[Valid, AccountRepository, Account] =
    for {
      _ <- credit(a.no, creditAmount)
      d <- debit(a.no, debitAmount)
    } yield d

  def composite(no: String,
                name: String,
                creditAmount: Amount,
                debitAmount: Amount): Kleisli[Valid, AccountRepository, Amount] =
    (for {
